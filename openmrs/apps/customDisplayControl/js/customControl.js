...
}]).directive('complications', ['$http', '$stateParams', 'appService', 'spinner', function ($http, $stateParams, appService, spinner) {
    var controller = function ($scope) {
        $scope.contentUrl = appService.configBaseUrl() + "/customDisplayControl/views/complications.html";
        $scope.title = $scope.config.title;
        ...
        spinner.forPromise(getResponseFromQuery().then(function (response) {
            $scope.complications = response.data;
            if ($scope.complications.length <= 0) {
                emitNoDataPresentEvent();
            } else {
                $scope.headings = _.keys($scope.complications[0]);
            }
        }));
    };
...
}]).directive('operativeReport', ['$http', '$stateParams', 'appService', 'spinner', function ($http, $stateParams, appService, spinner) {
    var controller = function ($scope) {
        $scope.contentUrl = appService.configBaseUrl() + "/customDisplayControl/views/operativeReport.html";
        $scope.title = $scope.config.title;
        ...
        var getResponseFromQuery = function () {
            var params = {
                patientUuid: $scope.patient.uuid,
                visitUuid: $stateParams.visitUuid,
                q: "bahmni.sqlGet.operativeReport",
                v: "full"
            };
            return $http.get('/openmrs/ws/rest/v1/bahmnicore/sql', {
                method: "GET",
        ...
        spinner.forPromise(getResponseFromQuery().then(function (response) {
            $scope.operativeReport = response.data;
            if ($scope.operativeReport.length <= 0) {
                emitNoDataPresentEvent();
            } else {
                $scope.headings = _.keys($scope.operativeReport[0]);
            }
        }));
    };
