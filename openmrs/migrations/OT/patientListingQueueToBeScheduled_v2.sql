...
GROUP BY
  ...
  LEFT JOIN(
    SELECT
      ...
      o.date_created As Date_updated
    FROM
      obs o
      INNER JOIN encounter e ON e.encounter_id = o.encounter_id
      AND o.voided IS FALSE
...
ORDER BY
  CASE
    WHEN pre_planned_Procedure.Date_updated IS NULL
    AND follow_planned_Procedure.Date_updated IS NULL THEN 1
    ELSE 0
  END ASC,
  CASE
    WHEN IFNULL(
      pre_planned_Procedure.Date_updated,
      '0000-00-00'
    ) >= IFNULL(
      follow_planned_Procedure.Date_updated,
      '0000-00-00'
    ) THEN pre_planned_Procedure.Date_updated
    ELSE follow_planned_Procedure.Date_updated
  END ASC;",
