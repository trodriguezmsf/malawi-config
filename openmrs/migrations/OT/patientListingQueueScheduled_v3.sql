...
VALUES ('emrapi.sqlSearch.otScheduledQueue',
        ...
        concat_ws(
                ...
                if(
                            IFNULL(
                                    pre_clinical_diagnosis.encounter_datetime,
                                    '0000-00-00'
                                ) >= IFNULL(
                                follow_clinical_diagnosis.encounter_datetime,
                                    '0000-00-00'
                                ),
                            pre_clinical_diagnosis.value,
                            NULL
                    ),
                if(
                            IFNULL(
                            ...
                            follow_clinical_diagnosis.value,
                            NULL
                    )
            )                                      AS `Diagnosis new`,
        concat_ws(
 ...
 FROM surgical_block sb
     ...
     AND pp.date_completed IS NULL
          ...
          LEFT JOIN(SELECT o.person_id,
       ...
       ) AS 'value', latest_encounter.encounter_datetime
                    FROM obs o
                    ...
                    GROUP BY o.person_id) pre_clinical_diagnosis ON pre_clinical_diagnosis.person_id = sa.patient_id
          LEFT JOIN(SELECT o.person_id,
                           GROUP_CONCAT(
                               DISTINCT (COALESCE(coded_fscn.name, coded_scn.name))
       ) AS 'value', latest_encounter.encounter_datetime
                    FROM obs o
                        ...
                        AND coded_scn.voided IS FALSE
                             INNER JOIN (SELECT o.person_id,
                                        ...
                                        ON latest_encounter.encounter_datetime = e.encounter_datetime
                                            AND latest_encounter.person_id = o.person_id
                    GROUP BY o.person_id) follow_clinical_diagnosis
                   ON follow_clinical_diagnosis.person_id = sa.patient_id
          LEFT JOIN(SELECT o.person_id,
                           GROUP_CONCAT(
                               DISTINCT (COALESCE(coded_fscn.name, coded_scn.name))
       ) AS 'value', latest_encounter.encounter_datetime
                    FROM obs o
                        ...
                        AND coded_scn.voided IS FALSE
                             INNER JOIN (SELECT o.person_id,
                                        ...
                                        ON latest_encounter.encounter_datetime = e.encounter_datetime
                                            AND latest_encounter.person_id = o.person_id
                    GROUP BY o.person_id) pre_planned_Procedure
                   ON pre_planned_Procedure.person_id = sa.patient_id
          LEFT JOIN (SELECT o.person_id,
                            GROUP_CONCAT(
                                DISTINCT (COALESCE (coded_fscn.name, coded_scn.name))
                        ) AS 'value', latest_encounter.encounter_datetime
                     FROM obs o
                         ...
                         AND coded_scn.voided IS FALSE
                              INNER JOIN (SELECT o.person_id,
                                         ...
                                         ON latest_encounter.encounter_datetime = e.encounter_datetime
                                             AND latest_encounter.person_id = o.person_id
                     GROUP BY o.person_id) follow_planned_Procedure ON follow_planned_Procedure.person_id = sa.patient_id
          LEFT JOIN (SELECT o.person_id,
                            GROUP_CONCAT(
                                DISTINCT (COALESCE (coded_fscn.name, coded_scn.name))
                        ) AS 'value'
