DELETE FROM global_property WHERE property = 'emrapi.sqlSearch.patientsAdmitToSupportiveCareWard';
SELECT uuid() INTO @uuid;
INSERT INTO global_property (`property`, `property_value`, `description`, `uuid`)
VALUES ('emrapi.sqlSearch.patientsAdmitToSupportiveCareWard',
"SELECT DISTINCT
  ...
  'Admit To Supportive Care Ward'                                         AS action,
...
WHERE v.date_stopped IS NULL AND cn.name = 'Admit to Supportive Care Ward' AND p.person_id NOT IN (SELECT patient_id
                                                                                   FROM bed_patient_assignment_map bpam
...
ORDER BY o.obs_datetime;",'Admit to Supportive Care Ward',@uuid);