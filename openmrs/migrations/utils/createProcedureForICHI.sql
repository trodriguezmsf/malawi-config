CREATE  PROCEDURE CREATE_REFERENCE_MAPPING_ICHI(IN concept_name VARCHAR(200), IN code VARCHAR(50), IN value VARCHAR(10))
BEGIN
    ...
    THEN
        INSERT INTO concept_reference_map (concept_reference_term_id, concept_map_type_id,creator,date_created,concept_id,uuid)
            (SELECT  ct.concept_reference_term_id,value,1,now(),cn.concept_id,uuid()
             FROM  concept_name cn,
